...
class ChordChain:
    ...
    def train(self, chord_sequences: Sequence[Sequence[ChordIndex]]) -> None:
        ...
        if self.vocab_size is not None and inferred_size > self.vocab_size:
            raise ValueError(f"sequence index {inferred_size - 1} exceeds vocab_size {self.vocab_size}")

        if self.counts is None:
            self.counts = np.zeros((size, size), dtype=np.int64)
            self.vocab_size = size
        elif self.counts.shape != (size, size):
            raise ValueError(f"counts shape {self.counts.shape} does not match vocab_size {size}")
        ...
        for seq in chord_sequences:
            for prev, curr in zip(seq, seq[1:]):
                ...
                if not (0 <= prev < size and 0 <= curr < size):
                    raise ValueError(f"chord index out of range [0, {size}): ({prev}, {curr})")
    ...
    def train_corpus(
        ...
        if self.vocab_size is None:
            ...
            raise ValueError(f"chordChain vocab_size {self.vocab_size} does not match chord_to_index size {vocab_len}")
        ...
        if self.counts is None or self.counts.sum() == 0:
            raise ValueError("cannot train on an empty corpus: no chord transitions were accumulated.")
    ...
    def normalize(self, alpha: float = 0.0) -> None:
        ...
        if self.counts.sum() == 0:
            raise RuntimeError("cannot normalize: no chord transitions were accumulated during training.")
        if alpha > 0:
            self.transition_matrix = laplace_smooth(self.counts, alpha)
            return
    ...
    def sample(self, current_chord_index: ChordIndex) -> ChordIndex:
        if self.transition_matrix is None:
            raise RuntimeError("cannot sample: transition matrix is not normalized. call normalize() on ChordChain after training.")
        if self.vocab_size is None:
        ...
        if row.sum() <= 0:
            ...
            raise RuntimeError(f"cannot sample: chord index {current_chord_index} has no outgoing transitions (row sum is zero). Apply smoothing before sampling if this state should be reachable during generation.")
    ...
    # load a ChordChain saved with save()
    @classmethod
